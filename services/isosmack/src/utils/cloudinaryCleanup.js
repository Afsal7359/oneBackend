...
// The upload middleware configures the SDK too, but models load first in some
// entry points. Configuring here as well is harmless — it is one global singleton.
if (isConfigured()) {
  cloudinary.config({
...
function collectAssets(value, out = new Map(), depth = 0) {
  ...
  for (const [key, val] of Object.entries(source)) {
    ...
    if (typeof val === 'string' && PUBLIC_ID_KEY_RE.test(key)) {
      const id = val.trim();
      // `local:` ids come from the on-disk fallback used when Cloudinary keys
      // are absent. They are swept the same way, just with fs.unlink.
      if (id && !/^https?:/i.test(id)) {
        addAsset(out, id, id.startsWith('local:') ? 'local' : (/video/i.test(key) ? 'video' : 'image'), null);
      }
      continue;
    }
...
function destroyAssets(assets, context = '') {
  ...
  if (!isConfigured() && !assets.some((a) => a.resourceType === 'local')) return;

  for (const asset of assets) {
    if (seenRecently(assetKey(asset))) continue;
    enqueue(async () => {
      try {
        if (asset.resourceType === 'local') {
          const rel = asset.publicId.slice('local:'.length);
